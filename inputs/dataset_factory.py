...
def chose_random_lines(list_lines: list[list], num_lines: int) -> list[list]:
    ...
    if num_lines >= len(list_lines):
        sampled = list_lines.copy()
    else:
        sampled = random.sample(list_lines, num_lines)
    
    # Reindex from 1 to num_lines
    for i, line in enumerate(sampled, start=1):
        line[0] = i
...
def main():
    ...
    filename = BENCHMARKS_DIR / "small_radius_to_small_radius_DL"
